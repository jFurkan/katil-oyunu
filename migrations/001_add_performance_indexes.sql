...
-- ÖNEMLI: CONCURRENTLY kullanımı production'da lock yaratmaz
-- NOT: Index zaten varsa "already exists" hatası alırsınız, sorun değil
-- NOT: CONCURRENTLY transaction block DIŞINDA çalışır
...
CREATE INDEX CONCURRENTLY idx_clues_team_id ON clues(team_id);

-- team_badges.team_id - getAllTeams() ve getTeamBadges()'de kullanılıyor
CREATE INDEX CONCURRENTLY idx_team_badges_team_id ON team_badges(team_id);

-- team_badges.badge_id - JOIN için
CREATE INDEX CONCURRENTLY idx_team_badges_badge_id ON team_badges(badge_id);

-- users.team_id - getUsersByTeam() ve JOIN'lerde kullanılıyor
CREATE INDEX CONCURRENTLY idx_users_team_id ON users(team_id);

-- users.socket_id - disconnect'te WHERE socket_id kullanılıyor
CREATE INDEX CONCURRENTLY idx_users_socket_id ON users(socket_id);

-- team_messages.team_id - getTeamMessages()'de kullanılıyor
CREATE INDEX CONCURRENTLY idx_team_messages_team_id ON team_messages(team_id);

-- team_messages.target_team_id - mesaj filtreleme için
CREATE INDEX CONCURRENTLY idx_team_messages_target_team_id ON team_messages(target_team_id);

-- team_messages.user_id - admin stats'te JOIN için
CREATE INDEX CONCURRENTLY idx_team_messages_user_id ON team_messages(user_id);

-- murder_board_items.team_id - getMurderBoardData()'da WHERE kullanılıyor
CREATE INDEX CONCURRENTLY idx_murder_board_items_team_id ON murder_board_items(team_id);

-- murder_board_connections.team_id - getMurderBoardData()'da WHERE kullanılıyor
CREATE INDEX CONCURRENTLY idx_murder_board_connections_team_id ON murder_board_connections(team_id);
...
CREATE INDEX CONCURRENTLY idx_team_messages_team_created ON team_messages(team_id, created_at DESC);

-- clues: team_id ile birlikte created_at sıralaması
CREATE INDEX CONCURRENTLY idx_clues_team_created ON clues(team_id, created_at);

-- team_badges: team_id ile birlikte awarded_at sıralaması
CREATE INDEX CONCURRENTLY idx_team_badges_team_awarded ON team_badges(team_id, awarded_at DESC);

-- users: team_id + is_captain + created_at (getUsersByTeam sıralaması için)
CREATE INDEX CONCURRENTLY idx_users_team_captain_created ON users(team_id, is_captain DESC, created_at);
...
CREATE INDEX CONCURRENTLY idx_users_online ON users(online) WHERE online = TRUE;
...
CREATE INDEX CONCURRENTLY idx_team_messages_admin ON team_messages(created_at DESC) WHERE target_team_id = 'admin';

-- characters.visible_to_teams - Public karakterler
CREATE INDEX CONCURRENTLY idx_characters_visible ON characters(name) WHERE visible_to_teams = TRUE;
...
-- ========================================
-- 6. INDEX BOYUTLARINI KONTROL ET
-- ========================================
